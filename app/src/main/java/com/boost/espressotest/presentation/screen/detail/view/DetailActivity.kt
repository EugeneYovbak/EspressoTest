...
class DetailActivity : DaggerActivity(), DetailView {
    ...
    private fun setProductInfo(product: Product) {
        GlideApp.with(this).load(product.imageUrl).centerCrop().into(iv_product_image)
    ...
    override fun productLoadError(error: String?) {
        showToast(error ?: getString(R.string.error_database))
    }
    ...
    override fun productStatusUpdateError(error: String?) {
        showToast(error ?: getString(R.string.error_database))
    }
